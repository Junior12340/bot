...
# ==================== SOZLAMALAR ====================
BOT_TOKEN = "8311683221:AAFWy1J5sq-9-_Kdp5qf3c7kMl9upEQoj4k"  # ← YANGI BOT TOKEN
GROUP_CHAT_ID = -1003773765959  # ← YANGILANGAN SUPERGROUP ID

# Qo'shimcha sozlamalar
DAILY_LIMIT = 5              # Kunlik murojaat limiti
REMINDER_DAYS = 15           # Eslatma kunlari
DB_PATH = "murojaatlar.db"   # Database fayl nomi
MEDIA_PATH = "media_photos"  # Rasmlar papkasi
DEFAULT_IMAGE = "default_image.png"  # Default rasm (rasm yuborilmasa ishlatiladi)
...
async def skip_photo_callback(callback: CallbackQuery, state: FSMContext):
    """Rasmisiz davom etish"""
    await callback.answer()
    
    # MUHIM: callback.message ishlatmaslik! U bot xabari
    # O'rniga, callback obyektini o'zi bilan ishlash kerak
    # Lekin finish_murojaat Message kutadi, shuning uchun
    # fake message yaratamiz yoki funksiyani o'zgartiramiz
    
    # Eng yaxshi yechim: callback.message dan foydalanish,
    # lekin user_id ni callback.from_user dan olish
    
    # Callback message ga foydalanuvchi ma'lumotlarini qo'shamiz
    callback.message.from_user = callback.from_user
    
    await finish_murojaat(callback.message, state, photo_path=None)
...
async def finish_murojaat(message: Message, state: FSMContext, photo_path: str = None):
    ...
    try:
        ...
        # DEBUG: User ID ni tekshirish
        actual_user_id = message.from_user.id
        logger.info(f"🔍 DEBUG: user_id={actual_user_id}, username={message.from_user.username}, first_name={message.from_user.first_name}")
