...
async def finish_murojaat(message: Message, state: FSMContext, photo_path: str = None):
    ...
    try:
        ...
        logger.info(f"✅ Guruhga yuborildi: message_id={group_message_id}, rasm={'ha' if photo_path else 'yo\'q'}")
    ...
    try:
        ...
        try:
            ...
            if user_id == bot_info.id:
                logger.warning(f"⚠️ Bot o'ziga xabar yuborishga urinish! user_id={user_id}")
                await message.reply(
                    f"⚠️ <b>Diqqat!</b>\n\n"
                    f"Bu murojaat bot tomonidan yuborilgan (test?).\n"
                    f"Javob foydalanuvchiga yuborilmadi.\n\n"
                    f"Murojaat #{murojaat_id} javob berildi deb belgilandi.",
                    parse_mode="HTML"
                )
                return
